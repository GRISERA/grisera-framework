...
class RegisteredChannelRouter:
    ...
    async def get_registered_channel(self, registered_channel_id: Union[int, str], depth: int, response: Response):
        """
        Get registered channels from database. Depth attribute specifies how many models will be traversed to create the
        response.
        """

        get_response = self.registered_channel_service.get_registered_channel(registered_channel_id, depth)
    ...
    @router.delete("/registered_channels/{registered_channel_id}", tags=["registered channels"],
                   response_model=Union[RegisteredChannelOut, NotFoundByIdModel])
    async def delete_registered_channel(self, registered_channel_id: Union[int, str], response: Response):
        """
    ...
    @router.put("/registered_channels/{registered_channel_id}/relationships", tags=["registered channels"],
                response_model=Union[RegisteredChannelOut, NotFoundByIdModel])
    async def update_registered_channel_relationships(self, registered_channel_id: Union[int, str],
