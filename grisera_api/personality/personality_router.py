...
class PersonalityRouter:
    ...
    async def create_personality_big_five(self, personality: PersonalityBigFiveIn, response: Response, dataset_name: str):
        """
        Create personality big five model in database
        """

        create_response = self.personality_service.save_personality_big_five(personality, dataset_name)
    ...
    async def create_personality_panas(self, personality: PersonalityPanasIn, response: Response, dataset_name: str):
        """
    ...
    async def get_personality(self, personality_id: int, response: Response, dataset_name: str):
        """
        Get personality from database
        """

        get_response = self.personality_service.get_personality(personality_id, dataset_name)
    ...
    @router.delete("/personality/{personality_id}", tags=["personality"],
                   response_model=Union[PersonalityBigFiveOut, PersonalityPanasOut, NotFoundByIdModel])
    async def delete_personality(self, personality_id: int, response: Response, dataset_name: str):
        """
    ...
    async def update_personality_big_five(self, personality_id: int, personality: PersonalityBigFiveIn,
        ...
        """
        update_response = self.personality_service.update_personality_big_five(personality_id, personality, dataset_name)
    ...
    async def update_personality_panas(self, personality_id: int, personality: PersonalityPanasIn, response: Response, dataset_name: str):
        """
        Update personality panas model in database
        """
        update_response = self.personality_service.update_personality_panas(personality_id, personality, dataset_name)
        if update_response.errors is not None:
            response.status_code = 404 if type(update_response) == NotFoundByIdModel else 422
