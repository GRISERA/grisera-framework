...
class RegisteredDataRouter:
    ...
    async def get_registered_data(self, registered_data_id: Union[int, str], depth: int, response: Response):
        """
        Get registered data from database. Depth attribute specifies how many models will be traversed to create the
        response.
        """

        get_response = self.registered_data_service.get_registered_data(registered_data_id, depth)
    ...
    @router.delete("/registered_data/{registered_data_id}", tags=["registered data"],
                   response_model=Union[RegisteredDataOut, NotFoundByIdModel])
    async def delete_registered_data(self, registered_data_id: Union[int, str], response: Response):
        """
    ...
    @router.put("/registered_data/{registered_data_id}", tags=["registered data"],
                response_model=Union[RegisteredDataOut, NotFoundByIdModel])
    async def update_registered_data(self, registered_data_id: Union[int, str], registered_data: RegisteredDataIn,
                                     response: Response):
        """
