...
class ParticipantStateRouter:
    ...
    async def create_participant_state(self, participant_state: ParticipantStateIn, response: Response, dataset_name: str):
        """
        Create participant state in database
        """

        create_response = self.participant_state_service.save_participant_state(participant_state, dataset_name)
    ...
    async def get_participant_state(self, participant_id: int, response: Response, dataset_name: str):
        """
        Get participant state from database
        """

        get_response = self.participant_state_service.get_participant_state(participant_id, dataset_name)
    ...
    @router.delete("/participant_state/{participant_state_id}", tags=["participant state"],
                   response_model=Union[ParticipantStateOut, NotFoundByIdModel])
    async def delete_participant_state(self, participant_state_id: int, response: Response, dataset_name: str):
        """
        Delete participant state from database
        """
        get_response = self.participant_state_service.delete_participant_state(participant_state_id, dataset_name)
        if get_response.errors is not None:
            response.status_code = 404
    ...
    async def update_participant_state_relationships(self, participant_state_id: int,
                                                     participant_state: ParticipantStateRelationIn, response: Response, dataset_name: str):
        """
