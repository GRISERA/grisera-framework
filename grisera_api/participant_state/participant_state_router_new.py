...
class ParticipantStateRouter:
    ...
    async def get_participant_state(self, participant_id: Union[int, str], depth: int, response: Response):
        """
        Get participant state from database. Depth attribute specifies how many models will be traversed to create the
        response.
        """

        get_response = self.participant_state_service.get_participant_state(participant_id, depth)
    ...
    @router.delete("/participant_state/{participant_state_id}", tags=["participant state"],
                   response_model=Union[ParticipantStateOut, NotFoundByIdModel])
    async def delete_participant_state(self, participant_state_id: Union[int, str], response: Response):
        """
    ...
    @router.put("/participant_state/{participant_state_id}", tags=["participant state"],
                response_model=Union[ParticipantStateOut, NotFoundByIdModel])
    async def update_participant_state(self, participant_state_id: Union[int, str], participant_state:
                                       ParticipantStatePropertyIn, response: Response):
        """
    ...
    @router.put("/participant_state/{participant_state_id}/relationships", tags=["participant state"],
                response_model=Union[ParticipantStateOut, NotFoundByIdModel])
    async def update_participant_state_relationships(self, participant_state_id: Union[int, str],
