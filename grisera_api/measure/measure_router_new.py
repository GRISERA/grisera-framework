...
class MeasureRouter:
    ...
    async def get_measure(self, measure_id: Union[int, str], depth: int, response: Response):
        """
    ...
    @router.put("/measures/{measure_id}", tags=["measures"],
                response_model=Union[MeasureOut, NotFoundByIdModel])
    async def update_measure(self, measure_id: Union[int, str], measure: MeasurePropertyIn, response: Response):
        """
    ...
    @router.put("/measures/{measure_id}/relationships", tags=["measures"],
                response_model=Union[MeasureOut, NotFoundByIdModel])
    async def update_measure_relationships(self, measure_id: Union[int, str], measure: MeasureRelationIn,
                                           response: Response):
        """
        Update measure relations in database
        """
        update_response = self.measure_service.update_measure_relationships(measure_id, measure)
        if update_response.errors is not None:
            response.status_code = 404
