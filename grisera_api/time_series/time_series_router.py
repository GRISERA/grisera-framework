...
class TimeSeriesRouter:
    ...
    @router.get("/time_series/{time_series_id}", tags=["time series"],
                response_model=Union[TimeSeriesOut, NotFoundByIdModel])
    async def get_time_series(self, time_series_id: int, response: Response, dataset_name: str,
        ...
        get_response = self.time_series_service.get_time_series(time_series_id, dataset_name, signal_min_value, signal_max_value)
    ...
    @router.delete("/time_series/{time_series_id}", tags=["time series"],
                   response_model=Union[TimeSeriesOut, NotFoundByIdModel])
    async def delete_time_series(self, time_series_id: int, response: Response, dataset_name: str):
        """
    ...
    async def update_time_series(self, time_series_id: int, time_series: TimeSeriesPropertyIn,
        ...
        """
        update_response = self.time_series_service.update_time_series(time_series_id, time_series, dataset_name)
    ...
    @router.put("/time_series/{time_series_id}/relationships", tags=["time series"],
                response_model=Union[TimeSeriesOut, NotFoundByIdModel])
    async def update_time_series_relationships(self, time_series_id: int, time_series: TimeSeriesRelationIn, response: Response, dataset_name: str):
        """
        Update time series relations in database
        """
        update_response = self.time_series_service.update_time_series_relationships(time_series_id, time_series, dataset_name)
        if update_response.errors is not None:
            response.status_code = 404
