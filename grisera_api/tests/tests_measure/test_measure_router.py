...
class TestMeasureRouter(unittest.TestCase):

    @mock.patch.object(MeasureService, 'save_measure')
    def test_create_measure_without_error(self, save_measure_mock):
        save_measure_mock.return_value = MeasureOut(measure_name="ECG", data_type="Test", range="Unknown", id=1)
        response = Response()
        measure = MeasureIn(measure_name="ECG", data_type="Test", range="Unknown")
        measure_router = MeasureRouter()

        result = asyncio.run(measure_router.create_measure(measure, response))

        self.assertEqual(result, MeasureOut(measure_name="ECG", data_type="Test",
    ...
    def test_create_measure_with_error(self, save_measure_mock):
        save_measure_mock.return_value = MeasureOut(measure_name="ECG", data_type="Test",
        ...
        self.assertEqual(result, MeasureOut(measure_name="ECG", data_type="Test", range="Unknown",
