import unittest
import unittest.mock as mock

from measure_name.measure_name_model import MeasureNamesOut, BasicMeasureNameOut
from measure_name.measure_name_service import MeasureNameService
from graph_api_service import GraphApiService
from measure.measure_model import *
from measure.measure_service import MeasureService


class TestMeasureService(unittest.TestCase):

    @mock.patch.object(GraphApiService, 'create_node')
    @mock.patch.object(GraphApiService, 'create_relationships')
    @mock.patch.object(GraphApiService, 'create_properties')
    @mock.patch.object(MeasureNameService, 'get_measure_names')
    def test_save_measure_without_error(self, get_measure_names_mock, create_properties_mock,
                                        create_relationships_mock, create_node_mock):
        id_node = 1
        create_node_mock.return_value = {'id': id_node, 'properties': None, "errors": None, 'links': None}
        create_relationships_mock.return_value = {'id': 3, 'start_node': 2, 'end_node': 3, "errors": None, 'links': None}
        get_measure_names_mock.return_value = MeasureNamesOut(measure_names=[
            BasicMeasureNameOut(id=5, name="Test", type="Test"),
            BasicMeasureNameOut(id=4, name="Test2", type="Test2")])
        calls = [mock.call(start_node=id_node, end_node=5, name="hasMeasureName")]
        measure = MeasureIn(measure_name="Test", data_type="Test", range="Unknown")
        measure_service = MeasureService()

        result = measure_service.save_measure(measure)

        self.assertEqual(result, MeasureOut(measure_name="Test", data_type="Test", range="Unknown", id=id_node))
        create_node_mock.assert_called_once_with('Measure')
        get_measure_names_mock.assert_called_once()
        create_relationships_mock.assert_has_calls(calls)
        create_properties_mock.assert_called_once_with(id_node, measure)

    @mock.patch.object(GraphApiService, 'create_node')
    def test_save_measure_with_node_error(self, create_node_mock):
        create_node_mock.return_value = {'properties': None, "errors": ['error'], 'links': None}
        measure = MeasureIn(measure_name="Test", data_type="Test", range="Unknown")
        measure_service = MeasureService()

        result = measure_service.save_measure(measure)

        self.assertEqual(result, MeasureOut(measure_name="Test", data_type="Test", range="Unknown", errors=['error']))
        create_node_mock.assert_called_once_with('Measure')
