...
class TestObservableInformationRouterPost(unittest.TestCase):
    ...
    def test_create_observable_information_without_error(self, save_observable_information_mock):
        save_observable_information_mock.return_value = ObservableInformationOut(modality_id=2, live_activity_id=3, id=1)
        response = Response()
        ...
        result = asyncio.run(observable_information_router.create_observable_information(observable_information, response))

        self.assertEqual(result, ObservableInformationOut(modality_id=2, live_activity_id=3, id=1, links=get_links(router)))
    ...
    def test_create_observable_information_with_error(self, save_observable_information_mock):
        save_observable_information_mock.return_value = ObservableInformationOut(modality_id=2, live_activity_id=3,
                                                                errors={'errors': ['test']})
        response = Response()
        observable_information = ObservableInformationIn(modality_id=2, live_activity_id=3)
        observable_information_router = ObservableInformationRouter()
        ...
        self.assertEqual(result, ObservableInformationOut(modality_id=2, live_activity_id=3,
