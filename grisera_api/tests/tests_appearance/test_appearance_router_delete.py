...
class TestAppearanceRouterDelete(unittest.TestCase):
    ...
    def test_delete_appearance_without_error(self, delete_appearance_mock):
        appearance_id = 1
        delete_appearance_mock.return_value = AppearanceOcclusionOut(beard="Heavy", moustache="Heavy", id=appearance_id)
        response = Response()
    ...
    def test_delete_appearance_with_error(self, delete_appearance_mock):
        delete_appearance_mock.return_value = AppearanceOcclusionOut(beard="Heavy", moustache="Heavy",
                                                                     errors={'errors': ['test']})
        response = Response()
        appearance_id = 1
        appearance_router = AppearanceRouter()
