...
class TestAppearanceRouterPut(unittest.TestCase):
    ...
    def test_update_appearance_occlusion_without_error(self, update_appearance_occlusion_mock):
        ...
        result = asyncio.run(appearance_router.update_appearance_occlusion(appearance_id, appearance, response, dataset_name))
        ...
        update_appearance_occlusion_mock.assert_called_once_with(appearance_id, appearance, dataset_name)
    ...
    def test_update_appearance_somatotype_without_error(self, update_appearance_somatotype_mock):
        ...
        result = asyncio.run(appearance_router.update_appearance_somatotype(appearance_id, appearance, response, dataset_name))

        self.assertEqual(result, AppearanceSomatotypeOut(ectomorph=2.7, endomorph=1.6, mesomorph=3.8,
                                                         id=appearance_id, links=get_links(router)))
        update_appearance_somatotype_mock.assert_called_once_with(appearance_id, appearance, dataset_name)
        self.assertEqual(response.status_code, 200)
