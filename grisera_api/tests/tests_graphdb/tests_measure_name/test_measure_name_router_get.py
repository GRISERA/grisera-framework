...
class TestMeasureNameRouterGet(unittest.TestCase):
    ...
    def test_get_measure_name_without_error(self, get_measure_name_mock):
        ...
        result = asyncio.run(measure_name_router.get_measure_name(measure_name_id, response, database_name))

        self.assertEqual(result, MeasureNameOut(name="Familiarity", type="Addional emotions measure", id=measure_name_id, links=get_links(router)))
        get_measure_name_mock.assert_called_once_with(measure_name_id, database_name)
    ...
    def test_get_measure_name_with_error(self, get_measure_name_mock):
        ...
        self.assertEqual(result, MeasureNameOut(name="Familiarity", type="Addional emotions measure", errors={'errors': ['test']},  links=get_links(router)))
        get_measure_name_mock.assert_called_once_with(measure_name_id, database_name)
    ...
    def test_get_measure_name_nodes_without_error(self, get_measure_names_mock):
        ...
        result = asyncio.run(measure_name_router.get_measure_names(response, database_name))
