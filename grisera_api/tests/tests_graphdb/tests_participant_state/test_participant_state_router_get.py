...
class TestParticipantStateRouterGet(unittest.TestCase):
    ...
    def test_get_participant_state_without_error(self, get_participant_state_mock):
        ...
        result = asyncio.run(participant_state_router.get_participant_state(participant_state_id, response, database_name))

        self.assertEqual(result, ParticipantStateOut(age=5, links=get_links(router)))
        get_participant_state_mock.assert_called_once_with(participant_state_id, database_name)
    ...
    def test_get_participants_state_without_error(self, get_participant_states_mock):
        ...
        result = asyncio.run(participant_state_router.get_participant_states(response, database_name))
