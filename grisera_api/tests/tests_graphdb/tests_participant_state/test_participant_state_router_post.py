...
class TestParticipantStateRouterPost(unittest.TestCase):
    ...
    def test_create_participant_state_without_error(self, save_participant_state_mock):
        ...
        result = asyncio.run(participant_state_router.create_participant_state(participant_state, response, database_name))

        self.assertEqual(result, ParticipantStateOut(participant=ParticipantIn(name="Test Test", sex='male', identifier=5),
                                                     id=1, links=get_links(router)))
        save_participant_state_mock.assert_called_once_with(participant_state, database_name)
