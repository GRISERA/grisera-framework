...
class TestTimeSeriesRouterGet(unittest.TestCase):
    ...
    def test_get_time_series_without_error(self, get_time_series_mock):
        time_series_id = 1
        get_time_series_mock.return_value = SignalSeriesOut(type="Epoch", source="cos")
        response = Response()
        time_series_router = TimeSeriesRouter()

        result = asyncio.run(time_series_router.get_time_series(time_series_id, 0, response, 10, 20))

        self.assertEqual(result, SignalSeriesOut(type="Epoch", source="cos", links=get_links(router)))
        get_time_series_mock.assert_called_once_with(time_series_id, 0, 10, 20)
        self.assertEqual(response.status_code, 200)

    @mock.patch.object(TimeSeriesServiceGraphDB, 'get_time_series')
    def test_get_time_series_with_error(self, get_time_series_mock):
        get_time_series_mock.return_value = SignalSeriesOut(type="Epoch", source="cos", errors={'errors': ['test']})
        response = Response()
        time_series_id = 1
        time_series_router = TimeSeriesRouter()
    ...
    def test_get_time_series_nodes_without_error(self, get_time_series_nodes_mock):
        get_time_series_nodes_mock.return_value = SignalSeriesNodesOut(time_series_nodes=[
