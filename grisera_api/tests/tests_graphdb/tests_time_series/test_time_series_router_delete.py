...
class TestTimeSeriesRouterDelete(unittest.TestCase):
    ...
    def test_delete_signal_series_without_error(self, delete_signal_series_mock):
        time_series_id = 1
        delete_signal_series_mock.return_value = SignalSeriesOut(id=1, type="Epoch", source="cos")
        response = Response()
        time_series_router = TimeSeriesRouter()

        result = asyncio.run(time_series_router.delete_signal_series(time_series_id, response))

        self.assertEqual(result, SignalSeriesOut(id=1, type="Epoch", source="cos", links=get_links(router)))
        delete_signal_series_mock.assert_called_once_with(time_series_id)
    ...
    def test_delete_signal_series_with_error(self, delete_signal_series_mock):
        ...
        self.assertEqual(result, SignalSeriesOut(id=1, type="Epoch", source="cos",
