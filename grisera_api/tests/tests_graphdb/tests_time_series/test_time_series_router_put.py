...
class TestTimeSeriesRouterPut(unittest.TestCase):
    ...
    @mock.patch.object(TimeSeriesServiceGraphDB, 'update_signal_series')
    def test_update_signal_series_without_error(self, update_signal_series_mock):
        time_series_id = 1
        update_signal_series_mock.return_value = SignalSeriesOut(id=1, type="Epoch", source="cos")
        response = Response()
        time_series = SignalSeriesPropertyIn(id=1, type="Epoch", source="cos")
        time_series_router = TimeSeriesRouter()

        result = asyncio.run(time_series_router.update_signal_series(
        ...
        update_signal_series_mock.assert_called_once_with(time_series_id, time_series)
    ...
    @mock.patch.object(TimeSeriesServiceGraphDB, 'update_signal_series_relationships')
    def test_update_signal_series_relationships_without_error(self, update_signal_series_relationships_mock):
        id_node = 1
        update_signal_series_relationships_mock.return_value = SignalSeriesOut(id=1, type="Epoch", source="cos")
        response = Response()
        ...
        result = asyncio.run(time_series_router.
                             update_signal_series_relationships(id_node, time_series_in, response))

        self.assertEqual(result, time_series_out)
        update_signal_series_relationships_mock.assert_called_once_with(id_node, time_series_in)
        self.assertEqual(response.status_code, 200)
