...
class TestPersonalityRouterPost(unittest.TestCase):
    ...
    def test_create_personality_big_five_without_error(self, save_personality_big_five_mock):
        ...
        result = asyncio.run(personality_router.create_personality_big_five(personality, response, dataset_name))
        ...
        save_personality_big_five_mock.assert_called_once_with(personality, dataset_name)
    ...
    def test_create_personality_panas_without_error(self, save_personality_panas_mock):
        ...
        result = asyncio.run(personality_router.create_personality_panas(personality, response, dataset_name))

        self.assertEqual(result, PersonalityPanasOut(negative_affect=0.5, positive_affect=0.5, id=1,
                                                     links=get_links(router)))
        save_personality_panas_mock.assert_called_once_with(personality, dataset_name)
        self.assertEqual(response.status_code, 200)
