...
def return_activity_execution(*args, **kwargs):
    activity_execution_out = ActivityExecutionOut(activity='group', identifier=2, id=1)
...
class TestActivityExecutionRouter(unittest.TestCase):
    ...
    def test_create_activity_execution_with_error(self, save_activity_execution_mock):
        save_activity_execution_mock.return_value = ActivityExecutionOut(activity='group', identifier=2,
                                                                         errors={'errors': ['test']})
        response = Response()
        activity_execution = ActivityExecutionIn(activity='group', identifier=2)
        activity_execution_router = ActivityExecutionRouter()
