...
class TestObservableInformationPostService(unittest.TestCase):
    ...
    # def test_observable_information_service_without_error(self, mock_requests):
        # response = Response()
        # response._content = json.dumps({'id': 3, 'properties': None, "errors": None,
        #                                 'links': None}).encode('utf-8')
        # get_response = Response()
        # get_response._content = json.dumps({'nodes': [{'properties': [{'key': 'modality', 'value': 'body posture'}]},
        #                                               {'properties': [{'key': 'live_activity', 'value': 'muscles activity'}]}],
        #                                     'errors': None,
        #                                     'links': []}).encode('utf-8')
