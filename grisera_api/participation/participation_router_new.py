...
class ParticipationRouter:
    ...
    async def get_participation(self, participation_id: Union[int, str], depth: int, response: Response):
        """
        Get participation from database. Depth attribute specifies how many models will be traversed to create the
        response.
        """

        get_response = self.participation_service.get_participation(participation_id, depth)
    ...
    @router.delete("/participations/{participation_id}", tags=["participations"],
                   response_model=Union[ParticipationOut, NotFoundByIdModel])
    async def delete_participation(self, participation_id: Union[int, str], response: Response):
        """
    ...
    @router.put("/participations/{participation_id}/relationships", tags=["participations"],
                response_model=Union[ParticipationOut, NotFoundByIdModel])
    async def update_participation_relationships(self, participation_id: Union[int, str], participation: ParticipationIn,
                                                 response: Response):
        """
