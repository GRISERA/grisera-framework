...
class ActivityExecutionRouter:
    ...
    async def get_activity_execution(self, activity_execution_id: Union[int, str], depth: int, response: Response):
        """
        Get activity executions from database. Depth attribute specifies how many models will be traversed to create the
        response.
        """

        get_response = self.activity_execution_service.get_activity_execution(activity_execution_id, depth)
    ...
    @router.delete("/activity_executions/{activity_execution_id}", tags=["activity executions"],
                   response_model=Union[ActivityExecutionOut, NotFoundByIdModel])
    async def delete_activity_execution(self, activity_execution_id: Union[int, str], response: Response):
        """
    ...
    @router.put("/activity_executions/{activity_execution_id}", tags=["activity executions"],
                response_model=Union[ActivityExecutionOut, NotFoundByIdModel])
    async def update_activity_execution(self, activity_execution_id: Union[int, str],
    ...
    @router.put("/activity_executions/{activity_execution_id}/relationships", tags=["activity executions"],
                response_model=Union[ActivityExecutionOut, NotFoundByIdModel])
    async def update_activity_execution_relationships(self, activity_execution_id: Union[int, str],
