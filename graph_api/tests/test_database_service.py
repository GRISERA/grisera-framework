...
class DatabaseServiceTestCase(unittest.TestCase):
    ...
    def test_get_nodes_by_query(self, requests_mock):
        ...
        commit_body = {"statements": [{
            "statement": "MATCH (n_0)-[:`hasSignal`]->(n_1),(n_1)-[:`next`*0..]->(n_2),(n_3)-[:`startInSec`]->(n_2),(n_4)-[:`endInSec`]->(n_2),(n_0:`Time Series`),(n_1:`Signal Value`),(n_2:`Signal Value`),(n_3:`Timestamp`),(n_4:`Timestamp`) WHERE ID(n_0)=15 AND n_4.propertyA='valueA' AND n_4.propertyB='valueB' RETURN n_2,LABELS(n_2),n_3,LABELS(n_3),n_4,LABELS(n_4)"}]}
        query = NodeRowsQueryIn(
            nodes=[
                ...
                NodeQueryIn(label="Timestamp", result=True, parameters={"propertyA": "valueA", "propertyB": "valueB"}),
            ],
            relations=[
